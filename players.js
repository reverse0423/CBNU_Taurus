// players.js — (Reco #1 수정)
// - (Reco #1) ensureWarStyle에서 <style> 태그 주입 코드 (st.textContent) 완전 삭제
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function applyDerivedBat(p) {
    const AB = N(p.ab ?? p.AB ?? p.타수); const H = N(p.hits ?? p.h ?? p.H ?? p.안타); const _2B = N(p.double ?? p['2B'] ?? p['2루타']); const _3B = N(p.triple ?? p['3B'] ?? p['3루타']); const HR = N(p.hr ?? p.HR ?? p.홈런); const BB = N(p.bb ?? p.BB ?? p.볼넷 ?? p['4구']); const IBB = N(p.ibb ?? p.IBB ?? p.고의4구); const HBP = N(p.hbp ?? p.HBP ?? p.사구); const SF = N(p.sf ?? p.SF ?? p['희생플라이'] ?? p['희비']); const SH = N(p.sh ?? p.SH ?? p['희생번트'] ?? p['희번']); const PAi = N(p.pa ?? p.PA ?? p.타석); const PA = PAi > 0 ? PAi : (AB + BB + HBP + SF + SH); const _1B = Math.max(0, H - _2B - _3B - HR); const TB = _1B + 2 * _2B + 3 * _3B + 4 * HR; const AVG = div(H, AB); const OBP = div(H + BB + HBP, AB + BB + HBP + SF); const SLG = div(TB, AB); const OPS = OBP + SLG; const uBB = Math.max(0, BB - IBB); const wDen = (AB + BB - IBB + HBP + SF); const wNum = WOBA.uBB * uBB + WOBA.HBP * HBP + WOBA['1B'] * _1B + WOBA['2B'] * _2B + WOBA['3B'] * _3B + WOBA.HR * HR; const wOBA = div(wNum, wDen);
  ...
  function parseIPValue(v) {
    if (v == null || v === '') return 0; if (typeof v === 'number') { const f = Math.trunc(v), frac = v - f; if (Math.abs(frac - 0.1) < 1e-6) return f + 1 / 3; if (Math.abs(frac - 0.2) < 1e-6) return f + 2 / 3; return v; } const s = normalizeFractionStr(String(v)).trim(); const m1 = s.match(/^(\d+)(?:\s+(\d)\/(\d))?$/); if (m1) { const whole = +m1[1], num = m1[2] ? +m1[2] : 0, den = m1[3] ? +m1[3] : 1; return whole + (den ? num / den : 0); } const m2 = s.match(/^(\d+)\.(\d)$/); if (m2) { const whole = +m2[1], dec = +m2[2]; if (dec === 1) return whole + 1 / 3; if (dec === 2) return whole + 2 / 3; return +s; } const n = parseFloat(s); return isNaN(n) ? 0 : n;
  }
  function deriveIP(p) {
    const outs = N(p.outs ?? p.Outs ?? p.아웃); if (outs > 0) return outs / 3; const raw = p.innings ?? p.ip ?? p.IP ?? p.이닝; return parseIPValue(raw);
  }
  function applyDerivedPitch(p) {
    const ER = N(p.er ?? p.ER ?? p['자책점']); const ip = deriveIP(p); const ERA = div(ER * ERA_BASE, ip); p.innings = ip > 0 ? formatIP(ip) : (p.innings ?? p.ip ?? p.IP ?? p.이닝 ?? 0); p.era = r3(ERA); return p;
  }
  function formatIP(ip) {
    const w = Math.floor(ip), f = ip - w; let mark = 0; if (Math.abs(f - 1 / 3) < 1e-6) mark = 1; else if (Math.abs(f - 2 / 3) < 1e-6) mark = 2; else if (f > 0) { const snaps = [0, 1 / 3, 2 / 3]; let best = 0, bd = 1; snaps.forEach((s, i) => { const d = Math.abs(f - s); if (d < bd) { bd = d; best = i; } }); mark = best; } return `${w}${mark ? '.' + mark : ''}`;
  }
  ...
  function leagueBatConstants(rows) {
    let WNUM = 0, WDEN = 0;
    rows.forEach(p => {
      const AB = N(p.ab), H = N(p.hits), _2B = N(p.double), _3B = N(p.triple), HR = N(p.hr); const BB = N(p.bb), IBB = N(p.ibb), HBP = N(p.hbp), SF = N(p.sf); const _1B = Math.max(0, H - _2B - _3B - HR); const uBB = Math.max(0, BB - IBB); WNUM += WOBA.uBB * uBB + WOBA.HBP * HBP + WOBA['1B'] * _1B + WOBA['2B'] * _2B + WOBA['3B'] * _3B + WOBA.HR * HR; WDEN += (AB + BB - IBB + HBP + SF);
    });
  ...
  function leaguePitchConstants(rows) {
    let RUNS = 0, OUTS = 0;
    rows.forEach(p => {
      const ip = deriveIP(p) || 0; RUNS += N(p.runs); OUTS += Math.round(ip * 3);
    });
    const IP = OUTS / 3; const ra7 = IP > 0 ? RUNS * ERA_BASE / IP : 0; const repRA7 = ra7 * WAR_CFG.PitReplacementMult;
    return { ra7, repRA7, rpw: WAR_CFG.RunsPerWin };
  }

  // 개인 WAR
  function calcBatWAR(p, K) {
    const PA = N(p.pa); if (PA <= 0) return 0; const woba = N(p.woba); const wraa = ((woba - K.lgwoba) / K.wobaScale) * PA; const bsR = 0.2 * N(p.sb); const repl = WAR_CFG.BatReplacementPerPA * PA; const runs = wraa + bsR + repl; return runs / K.rpw;
  }
  function calcPitWAR(p, K) {
    const ip = deriveIP(p) || 0; if (ip <= 0) return 0; const ra7 = N(p.runs) * ERA_BASE / ip; const rar = (K.repRA7 - ra7) * (ip / ERA_BASE); return rar / K.rpw;
  }
  ...
  function fetchBatYear(year) { return fetch(`records_${year}.json`).then(r => r.ok ? r.json() : Promise.reject(year)).then(normalizeRecords); }
  function fetchPitYear(year) { return fetch(`pitching_${year}.json`).then(r => r.ok ? r.json() : Promise.reject(year)).then(normalizeRecords).catch(() => []); }
  ...
  function aggregateBattingAllPlayers(allArrays) {
    ...
    allArrays.forEach(arr => {
      arr.forEach(r => {
        ...
        const t = map.get(key) || { name: r.name, number: r.number ?? r.playerId ?? r.id, ab: 0, hits: 0, double: 0, triple: 0, hr: 0, bb: 0, ibb: 0, hbp: 0, sf: 0, sh: 0, RBI: 0, RUN: 0, sb: 0 };
        t.name = t.name || r.name; t.number = t.number ?? r.number ?? r.playerId ?? r.id; t.ab += N(r.ab ?? r.AB ?? r.타수); t.hits += N(r.hits ?? r.h ?? r.H ?? r.안타); t.double += N(r.double ?? r['2B'] ?? r['2루타']); t.triple += N(r.triple ?? r['3B'] ?? r['3루타']); t.hr += N(r.hr ?? r['HR'] ?? r['홈런']); t.bb += N(r.bb ?? r.BB ?? r['볼넷'] ?? r['4구']); t.ibb += N(r.ibb ?? r.IBB ?? r['고의4구']); t.hbp += N(r.hbp ?? r.HBP ?? r['사구']); t.sf += N(r.sf ?? r.SF ?? r['희생플라이'] ?? r['희비']); t.sh += N(r.sh ?? r.SH ?? r['희생번트'] ?? r['희번']); t.RBI += N(r.RBI ?? r.rbi ?? r['타점']); t.RUN += N(r.RUN ?? r.run ?? r.R ?? r['득점']); t.sb += N(r.sb ?? r.SB ?? r['도루']);
        map.set(key, t);
      });
    });
  ...
  function aggregatePitchingAllPlayers(allArrays) {
    ...
    allArrays.forEach(arr => {
      arr.forEach(r => {
        const key = pidKey(r); if (!key) return;
        const t = map.get(key) || { name: r.name, number: r.number ?? r.playerId ?? r.id, outs: 0, er: 0, runs: 0, so: 0, bb: 0, hbp: 0, games: 0, wins: 0, losses: 0, holds: 0, saves: 0 };
        t.name = t.name || r.name; t.number = t.number ?? r.number ?? r.playerId ?? r.id; const outs = N(r.outs ?? r.Outs ?? r.아웃); t.outs += outs > 0 ? outs : Math.round(deriveIP(r) * 3); t.er += N(r.er ?? r.ER ?? r['자책점']); t.runs += N(r.runs ?? r.R ?? r['실점']); t.so += N(r.so ?? r.SO ?? r['탈삼진']); t.bb += N(r.bb ?? r.BB ?? r['볼넷']); t.hbp += N(r.hbp ?? r.HBP ?? r['사구']); t.games += N(r.games ?? r.G ?? r['경기수'] ?? r['등판 경기 수']); t.wins += N(r.wins ?? r.W); t.losses += N(r.losses ?? r.L); t.holds += N(r.holds ?? r.HLD ?? r['홀드']); t.saves += N(r.saves ?? r.SV ?? r['세이브']);
        map.set(key, t);
      });
    });
  ...
  function loadWarRanking(year) {
    ...
    if (isCareer) {
      ...
      Promise.allSettled([Promise.allSettled(batJobs), Promise.allSettled(pitJobs)])
        ...
        .catch(() => { rankingList.innerHTML = '<div>통산 랭킹 집계 실패</div>'; });
      return;
    }
    // 단일 연도
    Promise.all([fetchBatYear(year).catch(() => []), fetchPitYear(year).catch(() => [])])
      ...
      }).catch(() => { rankingList.innerHTML = '<div>랭킹 로드 실패</div>'; });
  }

  function ensureWarStyle() {
    // (Reco #1) 스타일 주입 코드 (st.textContent = ...) 완전 삭제
    // 스타일은 style.css에서 전역으로 관리됩니다.
  }
  ...
  function renderWarTop(batRows, pitRows, label) {
    ...
    (batRows || []).forEach(r => {
      ...
      const name = r.name || `#${pid}`; const w = calcBatWAR(r, Kb);
      const slot = touch(pid, name); slot.batWAR += w; slot.total += w;
    });
    (pitRows || []).forEach(r => {
      const pid = r.playerId ?? parseInt(r.number, 10) ?? r.id; if (!pid) return;
      const name = r.name || `#${pid}`; const w = calcPitWAR(r, Kp);
      const slot = touch(pid, name); slot.pWAR += w; slot.total += w;
    });
      ...
      `<div id="war-banner">
         ...
         <table>
           ...
           <thead>
             <tr>
               <th>순위</th> <th>선수</th>
               <th class="val">Bat</th> <th class="val">pWAR</th> <th class="total-val">WAR</th>
             </tr>
           </thead>
           <tbody>
             ${rows.map((r, idx) => `
               <tr>
                 <td>${idx + 1}</td>
                 <td>
                   <a class="name-link" href="player.html?id=${r.pid}">#${r.pid} ${escapeHTML(r.name || '')}</a>
                 </td>
