// players.js — FINAL (선수 목록 + WAR 랭킹 배너 Top 15)
// - WAR = Bat WAR + pWAR (타자 wOBA 기반, 투수 RA7 기반)
// - 연도별 / 통산(드롭다운 'total'/'career') 지원
// - Top 15 테이블, 클릭 시 player.html?id=... 이동
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderWarTop(batRows, pitRows, label) {
    ...
    // 선수별 WAR 합산
    const byPid = new Map(); // pid -> {pid,name,batWAR,pWAR,total}
    ...
    // 렌더링 (배너 + 테이블)
    const banner =
      `<div id="war-banner" style="padding:10px 12px;border:1px solid #e5e7eb;border-radius:10px;background:#fff;margin:8px 0 12px 0;">
         <div style="font-weight:700;margin-bottom:8px;">${label} WAR Top 15</div>
         <div style="overflow:auto;">
           <table style="width:100%; border-collapse:collapse; min-width:560px;">
             <thead>
               <tr>
                 <th style="text-align:left;padding:6px 8px;border-bottom:1px solid #eee;">순위</th>
                 <th style="text-align:left;padding:6px 8px;border-bottom:1px solid #eee;">선수</th>
                 <th style="text-align:right;padding:6px 8px;border-bottom:1px solid #eee;">Bat WAR</th>
                 <th style="text-align:right;padding:6px 8px;border-bottom:1px solid #eee;">pWAR</th>
                 <th style="text-align:right;padding:6px 8px;border-bottom:1px solid #eee;">WAR</th>
               </tr>
             </thead>
             <tbody>
               ${rows.map((r,idx)=>`
                 <tr>
                   ...
                   <td style="padding:6px 8px;">
                     <a href="player.html?id=${r.pid}" style="text-decoration:none;color:#2563eb;">#${r.pid} ${escapeHTML(r.name||'')}</a>
                   </td>
                   <td style="padding:6px 8px;text-align:right;">${r.batWAR.toFixed(2)}</td>
                   <td style="padding:6px 8px;text-align:right;">${r.pWAR.toFixed(2)}</td>
