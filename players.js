// players.js — FINAL (WAR Logic Unified with player.js/records.js + Cache Buster)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function parseIPValue(v){ if(v==null||v==='') return 0; if(typeof v==='number'){const f=Math.trunc(v),frac=v-f;if(Math.abs(frac-0.1)<1e-6) return f+1/3;if(Math.abs(frac-0.2)<1e-6) return f+2/3;if(Math.abs(frac-0.33)<0.02) return f+1/3;if(Math.abs(frac-0.66)<0.02||Math.abs(frac-0.67)<0.02) return f+2/3; return v;} const s=normalizeFractionStr(String(v)).trim(); const m=s.match(/^(\d+)(?:\s+(\d)\/(\d))?$/); if(m){const whole=+m[1],num=m[2]?+m[2]:0,den=m[3]?+m[3]:1; return whole+(den?num/den:0);} const m2=s.match(/^(\d+)\.(\d+)$/); if(m2){const whole=+m2[1],dec=+('0.'+m2[2]); if(Math.abs(dec-0.1)<1e-6) return whole+1/3;if(Math.abs(dec-0.2)<1e-6) return whole+2/3;if(Math.abs(dec-0.33)<0.02) return whole+1/3;if(Math.abs(dec-0.66)<0.02||Math.abs(dec-0.67)<0.02) return whole+2/3; return +s;} const n=parseFloat(s); return isNaN(n)?0:n; }
  ...
  // ── 로더들 (★ 핵심: Date.now()를 추가하여 브라우저 캐시를 무력화) ──
  const numericYearsFromSelect = () => Array.from(yearSelect?.options || []).map(o => o.value).filter(v => /^\d{4}$/.test(v));
  
  function fetchBatYear(year){ return fetch(`records_${year}.json?t=${Date.now()}`).then(r=>{ if(!r.ok) throw new Error(r.status); return r.json(); }).then(normalizeRecords).catch(e=>{ console.warn(`Bat ${year} fail:`,e); return []; }); }
  function fetchPitYear(year){ return fetch(`pitching_${year}.json?t=${Date.now()}`).then(r=>{ if(!r.ok) throw new Error(r.status); return r.json(); }).then(normalizeRecords).catch(e=>{ console.warn(`Pit ${year} fail:`,e); return []; }); }
  ...
  function aggregatePitchingAllPlayers(allArrays){
    ...
    allArrays.forEach(arr => {
      ...
      arr.forEach(r => {
        ...
        t.so+=N(r.so); t.k+=N(r.k??r.so??r.SO); 
  ...
  function renderWarTop(batRows, pitRows, label) {
    ...
    const rowPid = r => String(r.playerId ?? r.number ?? r.id ?? r.name ?? '').trim();

    (batRows || []).forEach(r => {
      const pid = rowPid(r); if (!pid) return;
    ...
    const pRows = (pitRows || []).map(r => ({
      pid: rowPid(r),
      name: r.name || `#${rowPid(r)}`,
      pWAR: calcPitWAR(r, Kp)
    })).filter(r => r.pid && isFinite(r.pWAR)).sort((a, b) => b.pWAR - a.pWAR).slice(0, 10);
    ...
    const banner =
      `<div id="war-banner">
         ...
         <div style="font-weight:700;margin:14px 0 6px;">${label} pWAR Top 10</div>
         <table>
           <thead><tr><th>순위</th><th>투수</th><th class="total-val">pWAR</th></tr></thead>
           <tbody>
             ${pRows.map((r, idx) => `
               <tr>
                 <td>${idx + 1}</td>
                 <td><a class="name-link" href="player.html?id=${encodeURIComponent(r.pid)}">#${escapeHTML(r.pid)} ${escapeHTML(r.name || '')}</a></td>
                 <td class="total-val">${r.pWAR.toFixed(2)}</td>
               </tr>`).join('')}
           </tbody>
         </table>
       </div>`;
