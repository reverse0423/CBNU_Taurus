// players.js — FINAL (선수 목록 + WAR 랭킹 배너 Top 10, 모바일 한 화면 맞춤)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function ensureWarStyle() {
    ...
    const css = `
      #war-banner { font-size:13px; }
      #war-banner table { width:100%; border-collapse:collapse; table-layout:fixed; }
      #war-banner th, #war-banner td { padding:4px 6px; }
      #war-banner col.rank { width:42px; }
      #war-banner col.num { width:70px; }
      #war-banner a { display:inline-block; max-width:100%; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; text-decoration:none; color:#2563eb; }
      /* 작은 화면: 더 컴팩트하게 */
      @media (max-width: 480px) {
        #war-banner { font-size:12px; }
        #war-banner th, #war-banner td { padding:3px 4px; }
        #war-banner col.rank { width:32px; }
        #war-banner col.num  { width:56px; }
      }
    `;
  ...
  function renderWarTop(batRows, pitRows, label) {
    ...
    const banner =
      `<div id="war-banner" style="padding:8px 10px;border:1px solid #e5e7eb;border-radius:10px;background:#fff;margin:8px 0 12px 0;">
         ...
         <table>
           ...
           <thead>
             <tr>
               <th style="text-align:left;border-bottom:1px solid #eee; font-weight:600;">순위</th>
               <th style="text-align:left;border-bottom:1px solid #eee; font-weight:600;">선수</th>
               <th style="text-align:right;border-bottom:1px solid #eee; font-weight:600;">Bat</th>
               <th style="text-align:right;border-bottom:1px solid #eee; font-weight:600;">pWAR</th>
               <th style="text-align:right;border-bottom:1px solid #eee; font-weight:700;">WAR</th>
             </tr>
           </thead>
           <tbody>
             ${rows.map((r,idx)=>`
               <tr>
                 ...
                 <td>
                   <a href="player.html?id=${r.pid}">#${r.pid} ${escapeHTML(r.name||'')}</a>
                 </td>
                 <td style="text-align:right;">${r.batWAR.toFixed(2)}</td>
                 <td style="text-align:right;">${r.pWAR.toFixed(2)}</td>
