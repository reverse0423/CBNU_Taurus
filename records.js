// records.js — FINAL (CSV 제거, 리더보드 가로 고정, 이름 자동완성: 시작 일치)
//  - 파생지표 자동계산: AVG/OBP/SLG/OPS/wOBA/ISO
//  - 검색: '강' 입력 → '강'으로 시작하는 선수만 보이기 + 자동완성 드롭다운
//  - 리더보드: 표의 가로 스크롤과 무관하게 고정(별도 영역)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function sortAndRender(records) {
    ...
    // 리더보드(타격만, 연도 페이지에서) — 가로 스크롤과 분리된 고정 영역
    const leaders = document.getElementById('leaders');
    if (leaders) {
      if (currentType === 'bat' && yearSel.value !== 'career' && rows.length) {
        leaders.innerHTML = buildLeadersHTML(rows);
      } else {
        leaders.innerHTML = '';
      }
      ensureLeadersPlacement(); // ← 스크롤 컨테이너 밖으로 이동
    }
  ...
  function topN(rows, key, n, isRate, minPA) {
    const arr = rows.slice().filter(r => !isRate || Number(r.pa) >= (minPA || 20));
    arr.sort((a,b)=> (+b[key]||0) - (+a[key]||0));
    return arr.slice(0, n);
  }
  function buildLeadersHTML(rows) {
    const blocks = [
    ...
    return blocks.map(b => {
      const top = topN(rows, b.key, 3, b.rate, 20);
  ...
  function findHorizontalScroller(el) {
    ...
    while (cur && cur !== document.body) {
      ...
      const isScroller = (ox === 'auto' || ox === 'scroll');
      if (isScroller) return cur;
  ...
  function updateSuggest() {
    ...
    list.forEach(name => {
      ...
      item.addEventListener('mousedown', (e) => { // blur 전에 처리되도록 mousedown
        e.preventDefault();
