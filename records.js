// records.js — FINAL (Comparison Toggle: Bat/Pit support + FIP WAR + K)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 비교 모달 상태
  let selectedPlayers = [null, null]; // IDs
  let compareMode = 'bat';
  ...
  const pitFields = [ 
    { key: 'name', label: '투수명' }, { key: 'games', label: '등판' }, { key: 'innings', label: '이닝' },{ key: 'era', label: '방어율' }, { key: 'wins', label: '승' }, { key: 'losses', label: '패' },{ key: 'so', label: '탈삼진' }, { key: 'pwar', label: 'pWAR' } 
  ];
  ...
  // ★ loadData 수정 (항상 타격/투수 데이터 모두 로드하여 전역 변수에 저장)
  function loadData(year) {
    ...
    .then(([bData, pData]) => {
        ...
        // WAR 계산 (전체 데이터를 기반으로)
        if (bData.length > 0) {
            const Kb = leagueBatConstants(bData);
            bData.forEach(p => { p.war = calcBatWAR(p, Kb); });
        }
        if (pData.length > 0) {
            const Kp = leaguePitchConstants(pData);
            pData.forEach(p => { p.pwar = calcPitWAR(p, Kp); });
        }
  ...
  // ============================================================
  // ★ 선수 비교 (Comparison) 기능 로직
  // ============================================================
  
  if (btnCompare) { btnCompare.addEventListener('click', () => { modal.style.display = 'flex'; resetComparison(); }); }
  if (closeModalBtn) { closeModalBtn.addEventListener('click', () => { modal.style.display = 'none'; }); }
  if (resetCompareBtn) { resetCompareBtn.addEventListener('click', resetComparison); }
  if (compareSearch) { compareSearch.addEventListener('input', (e) => { const q = e.target.value.toLowerCase(); const sourceList = compareMode === 'bat' ? globalBatData : globalPitData; const filtered = sourceList.filter(r => String(r.name||'').toLowerCase().includes(q) || String(r.number||'').includes(q)); renderCompareList(filtered); }); }
  ...
  function resetComparison() {
    ...
    compareMode = currentType; // 기본 모드는 현재 페이지 타입
    ...
    selectPhase.style.display = 'flex'; resultPhase.style.display = 'none';
  ...
  function updateComparisonView() {
    ...
    if (!p1 || !p2) {
       compareTable.innerHTML = `<tr><td colspan="3" style="padding:20px; color:#888;">두 선수 모두 해당 모드(${compareMode === 'bat' ? '타자' : '투수'})의 기록이 존재해야 합니다.</td></tr>`;
       return;
    }
    ...
    if (compareMode === 'bat') {
      metrics = [
        { key: 'pa', label: '타석' }, { key: 'avg', label: '타율' },
        { key: 'obp', label: '출루율' }, { key: 'slg', label: '장타율' },
        { key: 'ops', label: 'OPS' }, { key: 'hits', label: '안타' },
        { key: 'hr', label: '홈런' }, { key: 'RBI', label: '타점' },
        { key: 'sb', label: '도루' }, { key: 'k', label: '삼진', lowerIsBetter: true },
        { key: 'war', label: 'WAR' }
      ];
    } else {
      metrics = [
        { key: 'games', label: '등판' }, { key: 'era', label: 'ERA', lowerIsBetter: true },
        { key: 'innings', label: '이닝' }, { key: 'wins', label: '승' },
        { key: 'so', label: '탈삼진' }, { key: 'bb', label: '볼넷', lowerIsBetter: true },
        { key: 'pwar', label: 'pWAR' }
      ];
    }

    let html = `<thead><tr><th style="width:35%">${p1.name}</th><th style="width:30%">항목</th><th style="width:35%">${p2.name}</th></tr></thead><tbody>`;
