...
// - [NEW] 투수 ERA 자동계산(자책·이닝 기반), 이닝 표기 통일( .1=⅓, .2=⅔ ), ERA_BASE 선택 가능
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function normalizeFractionStr(str=''){
    return String(str)
      .replace(/\u2153/g, ' 1/3') // ⅓
      .replace(/\u2154/g, ' 2/3') // ⅔
      .replace(/\u00BC/g, ' 1/4') // ¼ (거의 없지만 방어)
      .replace(/\u00BD/g, ' 1/2') // ½
      .replace(/\u00BE/g, ' 3/4'); // ¾
  }
  // "12.1", "12.2", "12 2/3", "12⅔" 등 → 실수형 IP
  function parseIPValue(v){
    if (v == null || v === '') return 0;
    if (typeof v === 'number') {
      const floor = Math.trunc(v);
      const frac  = v - floor;
      // .1≈⅓, .2≈⅔ 로 해석
      if (Math.abs(frac - 0.1) < 1e-6) return floor + 1/3;
      if (Math.abs(frac - 0.2) < 1e-6) return floor + 2/3;
      // 이미 소수로 들어온 경우(예: 12.3333) 그대로 사용
      return v;
    }
  ...
  // 실수형 IP → 표시용 "a.b"(b=0|1|2)
  function formatIP(ip){
    const whole = Math.floor(ip);
    const frac  = ip - whole;
    let mark = 0;
    if (Math.abs(frac - 1/3) < 1e-6) mark = 1;
    else if (Math.abs(frac - 2/3) < 1e-6) mark = 2;
    else if (frac > 0) {
      // 애매한 소수는 가장 가까운 ⅓ 단위로 스냅
      const snaps = [0, 1/3, 2/3];
      let best = 0, bestDiff = 1;
      snaps.forEach((s,i)=>{ const d=Math.abs(frac-s); if(d<bestDiff){best=i;bestDiff=d;} });
      mark = best;
    }
    return `${whole}${mark ? '.'+mark : ''}`;
  }
  function deriveIP(p){
    // outs 우선
