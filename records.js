...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (positionFilter) positionFilter.addEventListener('change', () => sortAndRender(lastRows));

  function updateFilterButtonText() { filterBtn.textContent = filterOn ? '✅ 규정타석' : '⚾ 전체 보기'; filterBtn.style.backgroundColor = filterOn ? '#f7edf2' : '#f9f9f9'; filterBtn.style.color = filterOn ? '#7a003c' : '#333'; }
  ...
  function renderList(records) {
      ...
      records.forEach(r => { 
          ...
          statsDiv.innerHTML = keys.map(key => { 
              ...
              const colorStyle = isHighlight ? 'color:#7a003c;' : '';
              const weightStyle = isHighlight ? 'font-weight:bold; color:#7a003c;' : '';
  ...
  function sortAndRender(records) {
    ...
    if (positionFilter && positionFilter.value !== 'all') {
      rows = rows.filter(r => {
        const pid = String(r.number ?? r.id ?? r.playerId ?? '').trim();
        const meta = playerMeta.get(pid) || {};
        const pos = String(meta.position || r.position || r.pos || '');
        return pos.includes(positionFilter.value);
      });
    }
  ...
  fetch(`players.json?t=${Date.now()}`)
    .then(r => r.ok ? r.json() : [])
    .then(players => {
      playerMeta = new Map((players || []).map(p => [String(p.id), p]));
      if (lastRows.length) sortAndRender(lastRows);
    })
    .catch(() => {});
