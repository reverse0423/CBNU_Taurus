// records.js — FINAL (career 자동집계 + CI 포함 + 이닝 정렬 보정)
// - 리더보드 규정타석: 연간 팀 '승/패' 경기수 × 1.5 (자동)
//   * 취소/연기/무 등은 제외합니다 (승/패만 집계)
// - career(통산)도 리더보드 표시 (규정타석 40 고정, career는 연도별 파일 합산으로 자동계산)
// - CSV 제거 / 리더보드 가로 스크롤과 분리 고정 / 이름 자동완성(시작 일치)
// - 파생지표 자동계산: AVG/OBP/SLG/OPS/wOBA/ISO
// - 투수 ERA 자동계산(자책·이닝 기반), 이닝 표기 통일( .1=⅓, .2=⅔ ), ERA_BASE 선택 가능
// - ERA 표시는 소수 둘째 자리(0.00)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function applyDerived(p) {
    ...
    const SF  = N(p.sf ?? p.SF ?? p['희생플라이'] ?? p['희비'] ?? p['희플']); // 별칭 보강
    const SH  = N(p.sh ?? p.SH ?? p['희생번트'] ?? p['희번']);
    // CI(타격/포수 방해) → PA에는 포함(AB/OBP에는 미포함)
    const CI  = N(p.ci ?? p.CI ?? p['타격방해'] ?? p['포수방해']);
  ...
  function aggregatePitching(allArrays){
    ...
    allArrays.forEach(arr => {
      arr.forEach(r => {
        ...
        if (outs > 0) addOuts = outs;
          ...
          const ip = deriveIP(r);
          addOuts = Math.round(ip * 3); // ⅓ 단위이므로 정수
        }
        t.outs   += addOuts;
    ...
    // 파생지표 재계산(outs를 우선 사용 → IP/ERA 산출)
  ...
  function sortAndRender(records) {
    ...
    rows.sort((a, b) => {
      // ★ 투수 '이닝'은 ⅓ 단위 실수로 비교 (문자열/십분의일 오해 방지)
    ...
    rows.forEach(r => {
      ...
      fields.forEach(f => {
        ...
        td.textContent = floatF.includes(f.key)
          ? (f.key === 'era' ? Number(v).toFixed(2) : Number(v).toFixed(3)) // ERA는 0.00, 타격은 0.000
          : v;
        tr.appendChild(td);
      });
  ...
  // 데이터 로드 (+타격/투수 자동계산 & 리더보드 규정타석 설정)
  function loadData(year) {
    // 리더보드 규정타석 먼저 계산
    const minPAp = getMinPAForYear(year).then(v => { minPAForLeaders = v; });

    if (year === 'career') {
      const aggP = (currentType==='bat')
        ? loadCareerAggregated('bat')
        : loadCareerAggregated('pit');

      Promise.all([aggP, minPAp])
        .then(([arr]) => renderTable(arr))
        .catch(err => {
    ...
    let file;
    if (currentType==='bat') {
      file = `records_${year}.json`;
    } else {
      file = `pitching_${year}.json`;
    }
