// player.js — (Reco #1, #4 수정)
// - (Reco #1) ensurePitchGrid에서 <style> 태그 주입 코드 (st.textContent) 완전 삭제
// - (Reco #4) showBatting/showPitching에서 상세 기록 렌더링 시 .stat-label, .stat-value 클래스 추가
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function formatIP(ip) {
    const w = Math.floor(ip); const f = ip - w; let mark = 0; if (Math.abs(f - 1 / 3) < 1e-6) mark = 1; else if (Math.abs(f - 2 / 3) < 1e-6) mark = 2; else if (f > 0) { const snaps = [0, 1 / 3, 2 / 3]; let best = 0, bestDiff = 1; snaps.forEach((s, i) => { const d = Math.abs(f - s); if (d < bestDiff) { best = i; bestDiff = d; } }); mark = best; } return `${w}${mark ? '.' + mark : ''}`;
  }
  function deriveIP(p) {
  ...
  function applyDerivedPitch(p) {
    const ER = N(p.er ?? p.ER ?? p['자책점']); const ip = deriveIP(p); const ERA = div(ER * ERA_BASE, ip); if (ip > 0) p.innings = formatIP(ip); else p.innings = p.innings ?? p.ip ?? p.IP ?? p.이닝 ?? 0; p.era = r3(ERA); return p;
  }
  ...
  function aggregateBattingAllPlayers(allArrays) {
    ...
    allArrays.forEach(arr => arr.forEach(r => {
      ...
      t.name = t.name || r.name; t.number = t.number ?? r.number ?? r.playerId ?? r.id; t.ab += N(r.ab ?? r.AB ?? r.타수); t.hits += N(r.hits ?? r.h ?? r.H ?? r.안타); t.double += N(r.double ?? r['2B'] ?? r['2루타']); t.triple += N(r.triple ?? r['3B'] ?? r['3루타']); t.hr += N(r.hr ?? r.HR ?? p.홈런); t.bb += N(r.bb ?? r.BB ?? p.볼넷 ?? p['4구']); t.ibb += N(r.ibb ?? r.IBB ?? p.고의4구); t.hbp += N(r.hbp ?? r.HBP ?? p.사구); t.sf += N(r.sf ?? r.SF ?? p['희생플라이'] ?? p['희비']); t.sh += N(r.sh ?? r.SH ?? p['희생번트'] ?? p['희번']); t.RBI += N(r.RBI ?? r.rbi ?? p['타점']); t.RUN += N(r.RUN ?? r.run ?? r.R ?? p['득점']); t.sb += N(r.sb ?? r.SB ?? p['도루']);
      map.set(key, t);
    }));
  ...
  function aggregatePitchingAllPlayers(allArrays) {
    ...
    allArrays.forEach(arr => arr.forEach(r => {
      ...
      t.name = t.name || r.name; t.number = t.number ?? r.number ?? r.playerId ?? r.id; const outs = N(r.outs ?? r.Outs ?? r.아웃); t.outs += outs > 0 ? outs : Math.round(deriveIP(r) * 3); t.er += N(r.er ?? r.ER ?? p['자책점']); t.runs += N(r.runs ?? r.R ?? p['실점']); t.so += N(r.so ?? r.SO ?? p['탈삼진']); t.bb += N(r.bb ?? r.BB ?? p['볼넷']); t.hbp += N(r.hbp ?? r.HBP ?? p['사구']); t.games += N(r.games ?? r.G ?? p['경기수'] ?? p['등판 경기 수']); t.wins += N(r.wins ?? r.W); t.losses += N(r.losses ?? r.L); t.holds += N(r.holds ?? r.HLD ?? p['홀드']); t.saves += N(r.saves ?? r.SV ?? p['세이브']);
      map.set(key, t);
    }));
  ...
  function renderPitchSummaryTiles(p) {
    ...
    const tiles = [
      ...
      { lab: '승', val: N(p.wins) }, { lab: '패', val: N(p.losses) },
      { lab: '탈삼진', val: N(p.so) }, { lab: '볼넷', val: N(p.bb) },
      { lab: '사구', val: N(p.hbp) }, { lab: '실점', val: N(p.runs) },
      { lab: '자책점', val: N(p.er) }, { lab: 'WAR', val: pWAR.toFixed(2) }
    ];
  ...
  function loadYear(year) {
    ...
    togglePitchGrid(currentMode === 'pit');
  ...
  function showBatting() {
    ...
    if (!rec) {
      ...
      [
        ...
        // (Reco #4) .stat-label, .stat-value 클래스 추가
  ...
  function computeWAR() {
    ...
    (batData || []).forEach(r => {
      ...
      const w = calcBatWAR(r, Kb); warByPid.set(pid, (warByPid.get(pid) || 0) + w); r._batWAR = w;
    });
    (pitData || []).forEach(r => {
      const pid = r.playerId ?? parseInt(r.number, 10) ?? r.id; if (!pid) return;
      const w = calcPitWAR(r, Kp); warByPid.set(pid, (warByPid.get(pid) || 0) + w); r._pitWAR = w;
    });

    const batRec = (batData || []).find(r => (r.playerId ?? parseInt(r.number, 10)) === playerId) || (batData || []).find(r => r.name === playerName);
    const pitRec = (pitData || []).find(r => (r.playerId ?? parseInt(r.number, 10)) === playerId) || (pitData || []).find(r => r.name === playerName);
