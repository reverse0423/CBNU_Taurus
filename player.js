// player.js — FINAL (통산 자동 집계 + 파생지표 반영, BABIP 제거)
// 프로필 페이지: 배팅 기여도만 반영 (팀 내 순위 포함)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function showBatting() {
    ...
    computeContrib(); // 팀 전체 대비 개인 순위 산출
  }
  ...
  function showPitching() {
    ...
    computeContrib(); // 배팅만 반영
  }

  // ── 배팅 기여도(팀 내 순위) ───────────────────────────────────
  function computeContrib() {
    const score = r => {
      const avg = +r.avg||0, woba= +r.woba||0, slg= +r.slg||0, ops= +r.ops||0;
      const hr  = +r.hr||0,  sb  = +r.sb||0,  RBI= +r.RBI||0;
      return 30*ops + 25*woba + 20*slg + 15*avg + 5*hr + 2*sb + 3*RBI;
    };

    // batData에는 선택 연도의 전체 선수(단일연도) 또는 통산 집계(팀 전체)가 들어있음
    const totalsByPid = new Map();
    batData.forEach(r => {
      const pid = r.playerId ?? parseInt(r.number,10) ?? r.id;
      if (!pid) return;
      totalsByPid.set(pid, (totalsByPid.get(pid) || 0) + score(r));
    });

    const total = totalsByPid.get(playerId) || 0;
    const sorted = Array.from(totalsByPid.values())
      .filter(v => v !== 0)
      .sort((a,b) => b - a);
