...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const sumYear = document.getElementById('sum-year');
  const sumOps = document.getElementById('sum-ops');
  const sumWar = document.getElementById('sum-war');
  const sumPitch = document.getElementById('sum-pitch');
  const careerHighlightList = document.getElementById('career-highlight-list');
  ...
  function showPitching() {
    ...
    if (!rec) {
      ...
      document.getElementById('p-so').textContent = N(rec.k ?? rec.so);
      ...
      [['등판','games'],['이닝','innings',(ipVal > 0 ? formatIP(ipVal) : '0')],['방어율','era',eraTxt],
       ['승','wins'],['패','losses'],['홀드','holds'],['세이브','saves'],['탈삼진','k'],
  ...
  function computeWAR() {
    ...
    updateSummaryCards({ batWAR, pWAR, totalWAR: tWAR });
  }

  function updateSummaryCards(warValues) {
    const batRec = batData.find(r => pidKey(r) == playerId || r.name === playerName);
    const pitRec = pitData.find(r => pidKey(r) == playerId || r.name === playerName);
    const Kb = leagueBatConstants(batData || []);
    const Kp = leaguePitchConstants(pitData || []);
    const batWAR = warValues?.batWAR ?? (batRec ? calcBatWAR(batRec, Kb) : 0);
    const pWAR = warValues?.pWAR ?? (pitRec ? calcPitWAR(pitRec, Kp) : 0);
    if (sumYear) sumYear.textContent = yearSelect.value === 'total' ? '통산' : yearSelect.value;
    if (sumOps) sumOps.textContent = batRec && batRec.ops != null ? Number(batRec.ops).toFixed(3) : '-';
    if (sumWar) sumWar.textContent = (batWAR + pWAR).toFixed(2);
    if (sumPitch) {
      const era = pitRec && pitRec.era != null ? Number(pitRec.era).toFixed(2) : '-';
      sumPitch.textContent = pitRec ? `${era} / ${pWAR.toFixed(2)}` : '-';
    }
  }

  async function renderCareerHighlights() {
    if (!careerHighlightList) return;
    const years = numericYearsFromSelect();
    const batResults = await Promise.all(years.map(y => fetchYearFile(y, 'bat').then(rows => ({ year: y, rows: applyDerivedAll(rows) }))));
    const pitResults = await Promise.all(years.map(y => fetchYearFile(y, 'pit').then(rows => ({ year: y, rows: applyDerivedPitchAll(rows) }))));
    const careerBat = aggregateBattingAllPlayers(batResults.map(x => x.rows));
    const careerPit = aggregatePitchingAllPlayers(pitResults.map(x => x.rows));
    const myBat = careerBat.find(r => pidKey(r) == playerId || r.name === playerName);
    const myPit = careerPit.find(r => pidKey(r) == playerId || r.name === playerName);

    const seasonRows = batResults.map(({ year, rows }) => {
      const row = rows.find(r => pidKey(r) == playerId || r.name === playerName);
      if (!row) return null;
      const K = leagueBatConstants(rows);
      return { year, ops: N(row.ops), war: calcBatWAR(row, K), hits: N(row.hits), hr: N(row.hr) };
    }).filter(Boolean);
    const bestWar = seasonRows.slice().sort((a,b) => b.war - a.war)[0];
    const bestOps = seasonRows.filter(r => r.ops > 0).sort((a,b) => b.ops - a.ops)[0];

    const careerBatRankRows = careerBat.map(r => {
      const K = leagueBatConstants(careerBat);
      return { pid: pidKey(r), name: r.name, hits: N(r.hits), hr: N(r.hr), rbi: N(r.RBI ?? r.rbi), war: calcBatWAR(r, K) };
    });
    const Kp = leaguePitchConstants(careerPit);
    const careerPitRankRows = careerPit.map(r => ({ pid: pidKey(r), name: r.name, k: N(r.k ?? r.so), pwar: calcPitWAR(r, Kp) }));

    const items = [];
    if (bestWar) items.push(['최고 WAR 시즌', `${bestWar.year} (${bestWar.war.toFixed(2)})`]);
    if (bestOps) items.push(['최고 OPS 시즌', `${bestOps.year} (${bestOps.ops.toFixed(3)})`]);
    if (myBat) {
      items.push(['통산 안타', `${N(myBat.hits)}개${rankText(careerBatRankRows, 'hits')}`]);
      items.push(['통산 홈런', `${N(myBat.hr)}개${rankText(careerBatRankRows, 'hr')}`]);
      items.push(['통산 타점', `${N(myBat.RBI ?? myBat.rbi)}개${rankText(careerBatRankRows, 'rbi')}`]);
    }
    if (myPit) {
      items.push(['통산 탈삼진', `${N(myPit.k ?? myPit.so)}개${rankText(careerPitRankRows, 'k')}`]);
      items.push(['통산 pWAR', `${calcPitWAR(myPit, Kp).toFixed(2)}${rankText(careerPitRankRows, 'pwar')}`]);
    }
    careerHighlightList.innerHTML = items.length
      ? items.map(([label, value]) => `<div><span>${label}</span><strong>${value}</strong></div>`).join('')
      : '<p>표시할 커리어 기록이 없습니다.</p>';
  }

  function rankText(rows, key) {
    const sorted = rows.slice().sort((a,b) => b[key] - a[key]);
    const idx = sorted.findIndex(r => String(r.pid) === String(playerId));
    return idx >= 0 ? ` · 팀 ${idx + 1}위` : '';
  }
});
