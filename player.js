...
function loadRecord(playerId, year) {
  ...
  fetch(file)
    ...
    .then(data => {
      // 기록 찾기
      const rec = data.find(r => r.playerId !== undefined
        ? r.playerId === playerId
        : parseInt(r.number,10) === playerId);
      if (!rec) {
      ...
      // 상세 기록 리스트
      box.innerHTML = '';
      const fields = [['타석','pa'],['타수','ab'],['안타','hits'],['2루타','double'],
        ['3루타','triple'],['홈런','hr'],['4구','bb'],['사구','hbp'],['타점','RBI'],
        ['도루','sb'],['출루율','obp'],['장타율','slg'],['OPS','ops'],['wOBA','woba'],['BABIP','babip']];
      ...
      const salaryKRW = score * 500000;
      const salaryMan = Math.round(salaryKRW / 10000);

      // 팀 내 순위 계산
      const scoreList = data.map(r => {
        const rScore = 30*(r.ops||0) + 25*(r.woba||0) + 20*(r.slg||0)
          + 15*(r.avg||0) + 5*(r.hr||0) + 2*(r.sb||0) + 3*((r.RBI)||0);
        const pid = r.playerId !== undefined ? r.playerId : parseInt(r.number,10);
        return { pid, score: rScore };
      }).sort((a,b) => b.score - a.score);
      const rank = scoreList.findIndex(x => x.pid===playerId) + 1;

      // 연봉 및 순위 표시
      let salaryHTML = `Chat GPT 산정 예상 연봉: ${salaryMan.toLocaleString()}만원`;
      if (rank <= 5) salaryHTML += `<br>(팀 내 ${rank}위)`;
      salaryBox.innerHTML = salaryHTML;
    })
    .catch(err => {
      console.error(err);
