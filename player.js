// player.js — FINAL (투수 요약표 네모칸 스타일 적용 + WAR 표시)
// - 연도/통산 모두 화면에 로드된 팀 데이터로 리그 상수 계산 → WAR 산출
// - Bat WAR, pWAR, Total WAR(=합) 및 팀 내 순위 표시
// - 타자 요약(#stats-grid)처럼 "투수 요약표"를 네모칸 표 스타일로 추가(#pstats-grid)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const grid       = document.getElementById('stats-grid');  // 타자 요약 그리드
  ...
  // WAR 설정값(간이 WAR)
  const WAR_CFG = {
    WOBAScale: 1.15,        // wOBA → 득점 환산 스케일
    RunsPerWin: 7.5,        // 7이닝 환경의 승리당 득점
    BatReplacementPerPA: 0.033, // 대체수준(≈20 runs/600PA)
    PitReplacementMult: 1.20     // 대체 투수 RA7 = 리그RA7×1.20
  };
  ...
  function calcBatWAR(p, K){
    ...
    const bsR  = 0.2 * N(p.sb);                    // 간단 주루 보정
    const repl = WAR_CFG.BatReplacementPerPA * PA; // 대체수준
  ...
  function loadYear(year) {
    ...
    const pgridHide = document.getElementById('pstats-grid'); if (pgridHide) pgridHide.style.display='none';
  ...
  // ── 투수 요약표(네모칸) 생성/스타일 ───────────────────────────
  function ensurePitchGridStyled(){
    let wrap = document.getElementById('pstats-grid');
    if (wrap) return wrap;
  ...
  #pstats-grid .th{
    ...
    padding:10px 8px;text-align:center;font-weight:700;color:#374151;
    white-space:nowrap;font-size:clamp(12px,3.6vw,14px)
  }
  #pstats-grid .td{
    padding:12px 8px;text-align:center;font-variant-numeric:tabular-nums;
    white-space:nowrap;font-size:clamp(12px,3.6vw,14px)
  }

  /* 작은폰: 480px↓ 에서는 3열로 재배치 (스크롤 없이 보기 좋게) */
  @media (max-width:480px){
    #pstats-grid .row{ grid-template-columns:repeat(3, minmax(0,1fr)); }
  }
  /* 초소형폰: 360px↓ 에서는 2열 + 간격/패딩 축소 */
  @media (max-width:360px){
    #pstats-grid .row{ grid-template-columns:repeat(2, minmax(0,1fr)); gap:8px }
    #pstats-grid .th{ padding:8px 6px }
    #pstats-grid .td{ padding:10px 6px }
  }
`;
    ...
    const R1 = [
      ['이닝','p_ip'], ['방어율','p_era'], ['승','p_w'], ['패','p_l'], ['탈삼진','p_so']
    ];
    const R2 = [
      ['볼넷','p_bb'], ['사구','p_hbp'], ['실점','p_runs'], ['자책점','p_er'], ['WAR','p_war']
    ];

    const mkHeaderRow = arr =>
      `<div class="row head">${arr.map(([lab])=>`<div class="th">${lab}</div>`).join('')}</div>`;
    const mkValueRow  = arr =>
      `<div class="row vals">${arr.map(([_,id])=>`<div class="td" id="${id}">-</div>`).join('')}</div>`;

    wrap.innerHTML = mkHeaderRow(R1) + mkValueRow(R1) + mkHeaderRow(R2) + mkValueRow(R2);

    // 타자 요약(#stats-grid) 바로 아래에 삽입
    (grid?.parentNode || document.body).insertBefore(wrap, grid ? grid.nextSibling : null);
  ...
  function showBatting() {
    // 투수 요약표 숨김
    const pgrid = document.getElementById('pstats-grid');
    ...
    computeWAR(); // ← WAR 계산/표시
  }

  // ── 투구 표시(UI용: 네모칸 요약 + 상세) ───────────────────────
  function showPitching() {
    const pgrid = ensurePitchGridStyled();  // 네모칸 스타일 보장
    ...
    put('p_ip',   rec.innings ?? formatIP(deriveIP(rec)));
    put('p_era',  (rec.era!=null?Number(rec.era).toFixed(2):'0.00'));
    put('p_w',    N(rec.wins ?? rec.W));
    put('p_l',    N(rec.losses ?? rec.L));
    put('p_so',   N(rec.so ?? rec.SO));
    put('p_bb',   N(rec.bb ?? rec.BB));
    put('p_hbp',  N(rec.hbp ?? rec.HBP));
    put('p_runs', N(rec.runs ?? rec.R));
    put('p_er',   N(rec.er ?? rec.ER));
    put('p_war',  pWAR.toFixed(2));
