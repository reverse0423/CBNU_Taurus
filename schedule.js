// schedule.js — FINAL (Anti-Cache + 2026 Auto Detect + Filter)
var scheduleData = {};
...
function populateYearSelector() {
  var sel = document.getElementById('yearSelector');
  var typeSel = document.getElementById('typeSelector'); // 필터 선택자
  ...
  // 연도나 경기 종류가 바뀔 때마다 화면을 다시 그리도록 이벤트 추가
  sel.addEventListener('change', function(){ renderSchedule(sel.value); });
  if(typeSel) {
    typeSel.addEventListener('change', function(){ renderSchedule(sel.value); });
  }
}
...
function renderSchedule(year, focusId) {
  ...
  // 현재 선택된 경기 종류(필터) 가져오기
  var currentType = document.getElementById('typeSelector') ? document.getElementById('typeSelector').value : '전체';
  ...
  if (year === '전체') {
    ...
    var summaryLines = years.map(function(y){
      ...
      games.forEach(function(g){ 
        // 필터링 로직: '대학교' 포함 여부 확인
        var isUniv = (g.opponent || '').indexOf('대학교') !== -1;
        if (currentType === '대학부' && !isUniv) return; 
        if (currentType === '사회인' && isUniv) return;  

        if(g.result==='승') w++; else if(g.result==='패') l++; else if(g.result==='무') t++; 
      });
  ...
  var filteredGames = games.filter(function(game) {
    var isUniv = (game.opponent || '').indexOf('대학교') !== -1;
    if (currentType === '대학부') return isUniv;
    if (currentType === '사회인') return !isUniv;
    return true; 
  });

  if (filteredGames.length === 0) {
    container.innerHTML = '<div style="text-align:center; padding:40px; color:#888;">해당 조건에 맞는 일정이 없습니다.</div>';
    return;
  }
  ...
  filteredGames.forEach(function(game){
    ...
    // 경기 종류 라벨 추가
    var isUnivGame = opponent.indexOf('대학교') !== -1;
    var badgeText = isUnivGame ? '🎓대학부' : '⚾사회인';
    var badgeStyle = 'font-size: 11px; padding: 3px 6px; border-radius: 999px; margin-left: 6px; background-color: #f6eef3; color: #7a003c; font-weight: 800;';

    box.innerHTML =
      '<div class="date">'+date+time+'<span style="'+badgeStyle+'">'+badgeText+'</span></div>'+
