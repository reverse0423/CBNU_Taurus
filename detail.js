...
(function () {
  ...
  (function injectStyle(){
    ...
    var css =
      '.tau-wrap{overflow-x:auto;-webkit-overflow-scrolling:touch;margin:8px -14px 14px;border-top:1px solid #e5e7eb;border-bottom:1px solid #e5e7eb;background:#fff;}'+
      '.tau-table{border-collapse:separate;border-spacing:0;width:max-content;min-width:720px;font-size:13px;line-height:1.35;table-layout:fixed;}'+
      '.tau-table caption{caption-side:top;text-align:left;font-weight:800;margin:10px 14px 8px;font-size:15px;color:#111 !important;}'+
      '.tau-table th,.tau-table td{border-bottom:1px solid #e5e7eb;border-right:1px solid #e5e7eb;padding:8px 10px;background:#fff;white-space:nowrap;color:#111;text-align:center;min-width:58px;}'+
      ...
      '.tau-table.sticky-first th.col1,.tau-table.sticky-first td.col1{position:sticky;left:0;z-index:8;min-width:152px;width:152px;text-align:left;}'+
      '.tau-table.sticky-first th.col1{z-index:12;background:#f3f4f6;}'+
      '.tau-table.sticky-first td.col1{background:#fff;border-right:2px solid #e5e7eb;}'+
      '.tau-table.sticky-first tbody tr:nth-child(even) td.col1{background:#fafafa;}'+
      ...
      '@media (max-width:480px){ .tau-wrap{margin-left:-14px;margin-right:-14px}.tau-table{min-width:680px;font-size:12px}.tau-table th,.tau-table td{padding:8px 9px;min-width:52px}.tau-table.sticky-first th.col1,.tau-table.sticky-first td.col1{min-width:142px;width:142px}.tau-table.sticky-cols th.col1,.tau-table.sticky-cols td.col1{min-width:44px;width:44px}.tau-table.sticky-cols th.col2,.tau-table.sticky-cols td.col2{min-width:140px;width:140px;left:44px} }'+
      '.tau-scoreboard{width:100% !important;min-width:0 !important;table-layout:fixed;font-size:13px;border-spacing:0;border:1px solid #e5e7eb;border-radius:8px;overflow:hidden;}'+
      ...
      '.row-sub td{background:#f0f4ff !important;color:#374151}.player{display:flex;align-items:center;gap:8px}.player .bo{color:#6b7280;min-width:20px;text-align:center}.player .name{font-weight:700}.player .sub{font-size:12px;color:#6b7280;margin-top:2px}.empty{padding:16px;color:#777;text-align:center;}'+
  ...
  function table(opts){
    var caption = opts.caption || '', heads = opts.heads || [], rowsData = opts.rows || [], scoreboard = !!opts.scoreboard, stickyCols = !!opts.stickyCols, stickyFirst = !!opts.stickyFirst;
    var cls = 'tau-table' + (scoreboard ? ' tau-scoreboard' : '') + (stickyCols ? ' sticky-cols' : '') + (stickyFirst ? ' sticky-first' : '');
    var thead = '<thead><tr>' + heads.map(function(t, i){
      let headClass = ''; if (i === 0 && stickyFirst) headClass = 'col1'; else if (i === 0 && stickyCols) headClass = 'col1'; else if (i === 1 && stickyCols) headClass = 'col2 left'; else if (i === 0 && !stickyCols) headClass = 'left'; else headClass = 'num';
    ...
    var tbody = '<tbody>' + rowsData.map(function(r){
      ...
      return '<tr class="' + trc + '">' + cells.map(function(c, i){
        let tdClass = ''; if (i === 0 && stickyFirst) tdClass = 'col1'; else if (i === 0 && stickyCols) tdClass = 'col1'; else if (i === 1 && stickyCols) tdClass = 'col2 left'; else if (i === 0 && !stickyCols) tdClass = 'left'; else tdClass = 'num';
  ...
  function sumBatting(list){
    const t = { h:0, hr:0, rbi:0, sb:0, bb:0, hbp:0, so:0 };
    (Array.isArray(list) ? list : []).forEach(p => {
      t.h += N(p?.hits); t.hr += N(p?.hr); t.rbi += N(p?.rbi); t.sb += N(p?.sb);
      t.bb += N(p?.bb); t.hbp += N(p?.hbp); t.so += N(p?.so);
    });
    return t;
  }

  function sumPitching(list){
    const t = { so:0, er:0, r:0, bb:0, hbp:0 };
    (Array.isArray(list) ? list : []).forEach(p => {
      t.so += N(p?.so); t.er += N(p?.er); t.r += N(p?.r); t.bb += N(p?.bb); t.hbp += N(p?.hbp);
    });
    return t;
  }

  function topBatter(list){
    const rows = (Array.isArray(list) ? list : []).slice();
    rows.sort((a,b) => (N(b?.hits)*4 + N(b?.rbi)*3 + N(b?.hr)*5 + N(b?.sb)) - (N(a?.hits)*4 + N(a?.rbi)*3 + N(a?.hr)*5 + N(a?.sb)));
    return rows[0] || null;
  }

  function renderGameSummary(game){
    const homeBat = sumBatting(game.batting?.home || []), awayBat = sumBatting(game.batting?.away || []);
    const homePit = sumPitching(game.pitching?.home || []), awayPit = sumPitching(game.pitching?.away || []);
    const candidates = [topBatter(game.batting?.home || []), topBatter(game.batting?.away || [])].filter(Boolean);
    candidates.sort((a,b) => (N(b?.hits)*4 + N(b?.rbi)*3 + N(b?.hr)*5 + N(b?.sb)) - (N(a?.hits)*4 + N(a?.rbi)*3 + N(a?.hr)*5 + N(a?.sb)));
    const mvp = candidates[0];
    return `
      <article class="summary-tile"><span>MVP 후보</span><strong>${mvp ? mvp.name : '-'}</strong><em>${mvp ? `${N(mvp.hits)}안타 ${N(mvp.rbi)}타점` : '기록 없음'}</em></article>
      <article class="summary-tile"><span>팀 안타</span><strong>${awayBat.h} : ${homeBat.h}</strong><em>원정 : 홈</em></article>
      <article class="summary-tile"><span>장타/도루</span><strong>${awayBat.hr + homeBat.hr} HR / ${awayBat.sb + homeBat.sb} SB</strong><em>양 팀 합계</em></article>
      <article class="summary-tile"><span>투수 요약</span><strong>${awayPit.so + homePit.so}K</strong><em>자책 ${awayPit.er + homePit.er}</em></article>`;
  }

  function renderShareCard(game, teams){
    const away = teams.away || {}, home = teams.home || {};
    const innings = Array.isArray(game.innings) ? game.innings : [];
    const awayRuns = innings.reduce((s, i) => s + N(i.away), 0);
    const homeRuns = innings.reduce((s, i) => s + N(i.home), 0);
    return `
      <div class="share-kicker">TAURUS GAME CARD</div>
      <div class="share-date">${safe(game.date, '')} ${safe(game.status, '')}</div>
      <div class="share-match">
        <span>${away.abbr || away.name || 'AWAY'}</span>
        <strong>${awayRuns} : ${homeRuns}</strong>
        <span>${home.abbr || home.name || 'HOME'}</span>
      </div>
      <div class="share-venue">${safe(game.venue, '')}</div>`;
  }

  function renderBatting(side, list){
    list = Array.isArray(list) ? list : []; if (list.length === 0) return '<div class="empty">타격 기록이 없습니다.</div>';
    var heads = ['선수','타석','타수','안타','타점','득점','2루타','3루타','홈런','4사구','도루','삼진'];
    var rows = list.map(function(p, idx){
      ...
      var nameHtml = { html: '<div class="player"><span class="bo">'+safe(p?.bo, idx + 1)+'</span><div><div class="name">'+name+'</div><div class="sub">'+(subText.join(' · '))+'</div></div></div>' };
      var cells = [ nameHtml, safe(p?.pa, 0), safe(p?.ab, 0), safe(p?.hits, 0), safe(p?.rbi, 0), runs, safe(p?.double, 0), safe(p?.triple, 0), safe(p?.hr, 0), fourBall, safe(p?.sb, 0), safe(p?.so, 0) ];
  ...
  function renderPitching(side, list){
    ...
    return table({ caption: `투수 (${side})`, heads: heads, rows: rows, stickyFirst: true });
  }
  ...
  (function run(){
    getJSON(DATA_URL).then(function(db){
      ...
      if ($('#game-summary')) $('#game-summary').innerHTML = renderGameSummary(game);
      if ($('#share-card')) $('#share-card').innerHTML = renderShareCard(game, teams);
      ...
      if ($('#detail-loading')) $('#detail-loading').style.display = 'none';
      if ($('#detail-page')) $('#detail-page').style.display = '';
